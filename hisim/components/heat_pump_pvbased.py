...
class HeatPump(cp.Component):
    ...
    def __init__(self,
        ...
        self.t_outC: cp.ComponentInput = self.add_input(self.ComponentName,
                                                     ...
                                                     Units.Celsius,
                                                     True)
        # self.dhi_output: cp.ComponentInput = self.add_input(self.ComponentName,
        #                                              self.DiffuseHorizontalIrradiance,
        #                                              LoadTypes.Irradiance,
        #                                              Units.Wm2,
        #                                              True)
        # self.ghi_output: cp.ComponentInput = self.add_input(self.ComponentName,
    ...
    def get_weather_default_connections( self ):
        ...
        #connections.append( cp.ComponentConnection( self.DiffuseHorizontalIrradiance, weather_classname, Weather.DiffuseHorizontalIrradiance ) )
        #connections.append( cp.ComponentConnection( self.DiffuseHorizontalIrradiance, weather_classname, Weather.GlobalHorizontalIrradiance ) )
        return connections
...
class HeatPumpController(cp.Component):
    ...
    def get_building_default_connections( self ):
        ...
        connections.append( cp.ComponentConnection( HeatPumpController.DiffuseHorizontalIrradiance, building_classname, Building.DiffuseHorizontalIrradiance))
        connections.append( cp.ComponentConnection( HeatPumpController.GlobalHorizontalIrradiance, building_classname, Building.GlobalHorizontalIrradiance))
        return connections
        ...
        if force_convergence:
            ...
            if DHI != 0 and (GHI / DHI) > 2:
                ...
                if self.controller_heatpumpmode == 'heating':
                    if t_m_old > (self.t_set_heating + self.offset):
                        self.controller_heatpumpmode = 'off'
                elif self.controller_heatpumpmode == 'cooling':
                    if t_m_old < (self.t_set_cooling - self.offset):
                        self.controller_heatpumpmode = 'off'
                elif self.controller_heatpumpmode == 'off':
                    if t_m_old < self.t_set_heating:
                        self.controller_heatpumpmode = 'heating'
                    elif t_m_old > self.t_set_cooling:
