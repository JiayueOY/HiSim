    ...
    def build(self, location, my_simulation_parameters: SimulationParameters):
        ...
        if cachefound:
            ...
            if seconds_per_timestep != 60:
                ...
                self.DNI: List[float] = DNI.resample(
                    str(seconds_per_timestep) + "S").mean().tolist()  # )  # type: ignore
...
def readTRY(location="Aachen", year=2015):
    ...
    if "NSRDB_15min" in filepath:
        ...
        try:
            ...
            # Force convert columns to integers to avoid floating point representation
