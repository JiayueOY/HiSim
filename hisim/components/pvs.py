...
from hisim.utils import get_weather_data_path
from hisim import log
from hisim.components.weather import Weather, readTRY
...
class PVSystem(cp.Component):
    ...
    def get_weather_default_connections(self):
        ...
        connections.append(
            cp.ComponentConnection(PVSystem.TemperatureOutside, weather_classname, Weather.TemperatureOutside))
        connections.append(
            cp.ComponentConnection(PVSystem.DirectNormalIrradiance, weather_classname, Weather.DirectNormalIrradiance))
        connections.append(cp.ComponentConnection(PVSystem.DirectNormalIrradianceExtra, weather_classname,
                                                  Weather.DirectNormalIrradianceExtra))
        connections.append(cp.ComponentConnection(PVSystem.DiffuseHorizontalIrradiance, weather_classname,
                                                  Weather.DiffuseHorizontalIrradiance))
        connections.append(cp.ComponentConnection(PVSystem.GlobalHorizontalIrradiance, weather_classname,
    ...
    def get_coordinates(self, location="Aachen", year=2019):
        ...
        filepath = get_weather_data_path(location)

        # get the geoposition

        if "NSRDB_15min" in filepath:
            # Read .csv file directly if the file is from NSRDB_15min
            ...
            self.index = pd.date_range(
                            "{}-01-01 00:00:00".format(year), periods=35040, freq="15T", tz="Europe/Berlin"
            )
        ...
        if file_exists:
            ...
            if len(self.output) != self.my_simulation_parameters.timesteps:
                raise Exception("Reading the cached PV values seems to have failed. Expected "
                                + str(self.my_simulation_parameters.timesteps) + " values, but got " + str(
                    len(self.output)))
        else:
...
#                             "Wind Direction": "Wdir"})
#
#             # data.index = data.index.tz_localize('UTC').tz_convert('Europe/Berlin')  # 转换为本地时区
#
#             # 删除原有的日期时间组件列
...
#             raise ValueError(f"Error parsing date in file: {filepath + '.csv'}") from e
#
...
#         location = {"name": location_name, "latitude": lat, "longitude": lon}
#
#         #read from .dat and calculate DNI etc.
#         # get data
#         data = pd.read_csv(
