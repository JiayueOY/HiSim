...
from hisim.components import heat_pump_timebased
...
def basic_household_timebased_explicit(my_sim, my_simulation_parameters: Optional[SimulationParameters] = None):
    """
    ...
    my_heat_pump_timebased_controller = heat_pump_timebased.HeatPumpController(t_air_heating=t_air_heating,
                                                           t_air_cooling=t_air_cooling,
                                                           offset=offset,
                                                           mode=hp_mode,
                                                           my_simulation_parameters=my_simulation_parameters)
    my_heat_pump_timebased_controller.connect_input(my_heat_pump_timebased_controller.TemperatureMean,
                                          my_building.ComponentName,
                                          my_building.TemperatureMean)
    my_heat_pump_timebased_controller.connect_input(my_heat_pump_timebased_controller.ElectricityInput,
                                          my_base_electricity_load_profile.ComponentName,
                                          my_base_electricity_load_profile.ElectricityOutput)
    my_sim.add_component(my_heat_pump_timebased_controller)

    my_heat_pump_timebased = heat_pump_timebased.HeatPump(manufacturer=hp_manufacturer,
                                      name=hp_name,
                                      min_operation_time=hp_min_operation_time,
                                      min_idle_time=hp_min_idle_time,
                                      my_simulation_parameters=my_simulation_parameters)
    my_heat_pump_timebased.connect_input(my_heat_pump_timebased.State,
                               my_heat_pump_timebased_controller.ComponentName,
                               my_heat_pump_timebased_controller.State)
    my_heat_pump_timebased.connect_input(my_heat_pump_timebased.TemperatureOutside,
                               my_weather.ComponentName,
                               my_weather.TemperatureOutside)

    my_sim.add_component(my_heat_pump_timebased)

    my_building.connect_input(my_building.ThermalEnergyDelivered,
                              my_heat_pump_timebased.ComponentName,
                              my_heat_pump_timebased.ThermalEnergyDelivered)
...
def basic_household_with_default_connections(my_sim, my_simulation_parameters: Optional[SimulationParameters] = None):
    ...
    my_heat_pump_timebased_controller = heat_pump_timebased.HeatPumpController(t_air_heating=t_air_heating,
                                                           ...
                                                           mode=hp_mode,
                                                           my_simulation_parameters=my_simulation_parameters)
    my_heat_pump_timebased_controller.connect_only_predefined_connections(my_building)
    ...
    my_heat_pump_timebased = heat_pump_timebased.HeatPump(manufacturer=hp_manufacturer,
                                      ...
                                      min_idle_time=hp_min_idle_time,
                                      my_simulation_parameters=my_simulation_parameters)
    my_heat_pump_timebased.connect_only_predefined_connections(my_weather, my_heat_pump_timebased_controller)
